...
/***************************************************************************
 *                
 *     dwDriverId | For most messages, <p dwDriverId> is the DWORD
 ...
 *
 *     | hDriver | This is the handle returned to the
 *     application by the driver interface.
 *          
 *     WORD | wMessage | The requested action to be performed. Message
 ...
 *
 *     LONG | lParam1 | Data for this message.  Defined separately for
 *     each message
 *
 *    LONG | lParam2 | Data for this message.  Defined separately for
 *     each message
 *
 ***************************************************************************/
LONG FAR PASCAL _loadds DriverProc(DWORD dwDriverID, HANDLE hDriver,
								   WORD wMessage, LONG lParam1, LONG lParam2)
{
	static char aszMixer[]	  = "MMMIXER.DLL";
    ...
    switch (wMessage) {
        ...
        case DRV_LOAD:
            ...
            */
			...
			if (GetModuleHandle(aszMixer)) {
                if (!hInstMix) {
			        hInstMix = LoadLibrary(aszMixer); // Bump count 
                    if (hInstMix > 32)
                       fhaveMix = TRUE;
                    else
				       D1("No Mixer onboard!  But Got handle??");
                    }
                }
       ...
       case DRV_CONFIGURE:
            ...
            */
            ...
            return (LRESULT)Config((HWND)LOWORD(lParam1), ghInstance);
